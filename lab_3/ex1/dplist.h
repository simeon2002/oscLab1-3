...
// Typedef the structure for easier use
 ...
 */
dplist_t *dpl_insert_at_index(dplist_t *list, element_t element, int index); //QUESTION is a dplist_t returned because
// if f.e. the list = NULL, then after inserting a certain value, you have to make the NULL pointer point towards the list witht he value.
// because I was thinking.. if we f.e. already had a list with existing nodes, there would basically be no need to return it
// as the lcoal variable will just insert a value into the node, but not change the address of the dplist_t type then?
// QUESTION --> DO WE START FROM INDEX 0 OR 1?
 ...
 * - the first list node has index 0. //QUESTION WHAT DOES INDEX 0 MEAN??
